...
use bitchat_core::{
    PeerId, ChannelTransportType, TransportTask,
    internal::{
        ChannelConfig, create_event_channel, create_effect_channel,
    },
};
...
fn test_nostr_config_builder() {
    ...
    assert!(config.relays.iter().any(|r| r.url == "wss://relay.example.com"));
    ...
    assert_eq!(config_with_key.private_key.as_ref().unwrap().public_key(), keys.public_key());
}
