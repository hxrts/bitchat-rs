...
use bitchat_core::{PeerId, BitchatError, BitchatResult, internal::RateLimitConfig};
use instant::Instant;
...
impl RateLimiter {
    ...
    pub fn check_message_allowed(&mut self, peer_id: PeerId) -> BitchatResult<()> {
        ...
        let peer_limits = self.peer_limits.entry(peer_id)
    ...
    fn cleanup_old_peers(&mut self) {
        let now = Instant::now();
        let cleanup_threshold = std::time::Duration::from_secs(self.config.window_duration_secs * 2);
        
        // Remove peers that haven't been active recently
        self.peer_limits.retain(|_, limits| {
            now.duration_since(limits.last_access) < cleanup_threshold
        });

        // If we still have too many peers, remove the least recently used ones
        if self.peer_limits.len() > self.config.max_tracked_peers {
            let mut peers_by_access: Vec<_> = self.peer_limits.iter()
