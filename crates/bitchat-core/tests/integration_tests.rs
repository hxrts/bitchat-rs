...
impl TestPeer {
    ...
    async fn send_message_to(
        ...
        self.delivery_tracker
            .track_message(message_id, recipient_id, payload.clone());
    ...
    async fn send_large_message_to(
        ...
        let fragments = MessageFragmenter::fragment_message(
            message.id, &payload, 512, // Small fragment size for testing
        )?;
...
async fn test_basic_message_exchange() {
    ...
    let message_id = alice
        .send_message_to(bob.peer_id(), "Hello, Bob!".to_string())
        .await
        .unwrap();
...
async fn test_message_fragmentation() {
    ...
    let packets = alice
        .send_large_message_to(bob.peer_id(), large_content.clone())
        .await
        .unwrap();
    ...
    assert_eq!(
        packets[packets.len() - 1].message_type,
        MessageType::FragmentEnd
    );
...
async fn test_multi_peer_scenario() {
    ...
    let msg_to_bob = alice
        .send_message_to(bob.peer_id(), "Hello, Bob!".to_string())
        .await
        .unwrap();
    let msg_to_charlie = alice
        .send_message_to(charlie.peer_id(), "Hello, Charlie!".to_string())
        .await
        .unwrap();
...
async fn test_delivery_retry_mechanism() {
    ...
    // Send message
    let message_id = alice
        .send_message_to(bob_id, "Test message".to_string())
        .await
        .unwrap();
...
async fn test_concurrent_fragmented_messages() {
    ...
    let packets1 = alice
        .send_large_message_to(bob.peer_id(), message1_content.clone())
        .await
        .unwrap();
    let packets2 = alice
        .send_large_message_to(bob.peer_id(), message2_content.clone())
        .await
        .unwrap();
    ...
    let mut received_contents: Vec<String> = bob
        .received_messages
        .iter()
...
async fn test_delivery_statistics() {
    ...
    let msg1 = alice
        .send_message_to(bob_id, "Message 1".to_string())
        .await
        .unwrap();
    let msg2 = alice
        .send_message_to(charlie_id, "Message 2".to_string())
        .await
        .unwrap();
    let msg3 = alice
        .send_message_to(bob_id, "Message 3".to_string())
        .await
        .unwrap();
