...
use bitchat_core::{
    PeerId, Command, AppEvent, ConnectionStatus, ChannelTransportType, Event, Effect, SystemTimeSource,
    internal::{
        ConnectionState, ConnectionEvent, SessionParams,
        MessageStore, ContentAddressedMessage, TimeSource, MessageId,
    },
};
...
fn create_test_message(from: PeerId, to: Option<PeerId>, content: &str, seq: u64) -> ContentAddressedMessage {
    ContentAddressedMessage::new(from, to, content.to_string(), seq)
}
...
fn test_connection_state_disconnected_to_discovering() {
    ...
    let transition = state.transition(ConnectionEvent::StartDiscovery { timeout_seconds: Some(60) });
    assert!(transition.is_ok());
    
    let transition = transition.unwrap();
    assert!(matches!(transition.new_state, ConnectionState::Discovering(_)));
...
fn test_connection_state_discovering_to_connecting() {
    ...
    let state = state.transition(ConnectionEvent::StartDiscovery { timeout_seconds: Some(60) })
        .unwrap().new_state;
    
    // Discover peer (stays in discovering state)
    let state = state.transition(ConnectionEvent::PeerDiscovered { 
    ...
    let transition = transition.unwrap();
    assert!(matches!(transition.new_state, ConnectionState::Connecting(_)));
}
...
fn test_connection_state_connecting_to_connected() {
    ...
    state = state.transition(ConnectionEvent::StartDiscovery { timeout_seconds: Some(60) })
        .unwrap().new_state;
    state = state.transition(ConnectionEvent::PeerDiscovered { 
        transport: ChannelTransportType::Ble, 
        signal_strength: Some(-50) 
    }).unwrap().new_state;
    ...
    state = state.transition(ConnectionEvent::InitiateConnection { 
        transport: ChannelTransportType::Ble,
        session_params,
    }).unwrap().new_state;
    ...
    let transition = transition.unwrap();
    assert!(matches!(transition.new_state, ConnectionState::Connected(_)));
}
...
fn test_connection_state_error_recovery() {
    ...
    state = state.transition(ConnectionEvent::ConnectionEstablished { 
        session_id: "test-session".to_string() 
    }).unwrap().new_state;
...
fn test_effect_planning_send_packet() {
    ...
    if let Effect::SendPacket { peer_id: p, data: d, transport: t } = effect {
        assert_eq!(p, peer_id);
        assert_eq!(d, data);
...
fn test_effect_planning_connection_initiation() {
    ...
    if let Effect::InitiateConnection { peer_id: p, transport: t } = effect {
        assert_eq!(p, peer_id);
        assert_eq!(t, transport);
    } else {
...
fn test_effect_planning_transport_discovery() {
    ...
    assert!(matches!(start_effect, Effect::StartTransportDiscovery { .. }));
...
fn test_command_structure() {
    ...
    if let Command::SendMessage { recipient, content: c } = send_cmd {
...
fn test_event_structure() {
    ...
    if let Event::MessageReceived { from, content: c, transport: t, message_id, recipient, timestamp, sequence } = event {
...
fn test_app_event_structure() {
    ...
    if let AppEvent::MessageReceived { from, content: c, timestamp: t } = app_event {
        assert_eq!(from, peer_id);
        assert_eq!(c, content);
