...
use bitchat_core::protocol::{BitchatPacket, MessageType, PacketFlags, PacketHeader};
...
fn test_canonical_header_size() {
    ...
    assert_eq!(
        header_bytes.len(),
        13,
        "Header must be exactly 13 bytes for canonical compatibility"
    );
...
fn test_canonical_fragment_format() {
    ...
    let fragment_header = FragmentHeader::new(
        ...
        5,                          // index
        10,                         // total
        MessageType::Message as u8, // original type
    );

    let header_bytes = fragment_header.to_bytes();
    assert_eq!(header_bytes.len(), 13, "Fragment header must be 13 bytes");

    // Verify structure matches canonical format exactly
    assert_eq!(
        u64::from_be_bytes(header_bytes[0..8].try_into().unwrap()),
        0x1234567890ABCDEF
    );
    assert_eq!(
        u16::from_be_bytes(header_bytes[8..10].try_into().unwrap()),
        5
    );
    assert_eq!(
        u16::from_be_bytes(header_bytes[10..12].try_into().unwrap()),
        10
    );
...
fn test_canonical_wire_format_compatibility() {
    ...
    assert_eq!(
        packet.header.message_type as u8,
        decoded.header.message_type as u8
    );
    ...
    assert!(
        encoded.len() < 50,
        "Encoded packet should be reasonably small"
    );
}
