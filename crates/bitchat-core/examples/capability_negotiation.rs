...
use bitchat_core::{
    Capability, CapabilityId, CapabilityManager, ImplementationInfo, NegotiationStatus, PeerId,
    ProtocolVersion, VersionHello,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Status after sending VersionHello: {:?}",
    ...
    println!(
        "Status after timeout (legacy detected): {:?}",
        rust_manager.get_negotiation_status(&canonical_peer)
    );
