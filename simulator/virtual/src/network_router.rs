...
impl NetworkRouter {
    ...
    pub fn new(config: NetworkRouterConfig, rng: Arc<Mutex<dyn RandomSource>>) -> (Self, mpsc::UnboundedSender<NetworkPacket>) {
    ...
    async fn process_outgoing_packet(&mut self, mut packet: NetworkPacket) -> Result<(), String> {
        ...
        match &self.config.profile {
            ...
            NetworkProfile::SlowWifi { latency_ms, jitter_ms } => {
                let jitter = self.rng.lock().unwrap().gen_range_u32(0, *jitter_ms + 1);
            ...
            NetworkProfile::Unreliable3G { packet_loss, reordering_chance, latency_ms, jitter_ms } => {
                ...
                if self.rng.lock().unwrap().gen_f32() < *packet_loss {
                    if self.config.enable_packet_logging {
                ...
                if self.rng.lock().unwrap().gen_f32() < *reordering_chance {
                    // Add extra delay for reordering
                    let extra_delay = self.rng.lock().unwrap().gen_range_u32(10, 100);
                    packet.deliver_at += Duration::from_millis(extra_delay as u64);
            ...
            NetworkProfile::Satellite { latency_ms, jitter_ms, occasional_outages } => {
                // Check for outages
                if *occasional_outages && self.rng.lock().unwrap().gen_f32() < 0.01 { // 1% chance of outage
                    if self.config.enable_packet_logging {
            ...
            NetworkProfile::MeshNetwork { hop_latency_ms, max_hops, partition_chance } => {
                ...
                if self.rng.lock().unwrap().gen_f32() < *partition_chance {
            ...
            NetworkProfile::Custom { 
                ...
                if self.rng.lock().unwrap().gen_f32() < *corruption_chance && !packet.data.is_empty() {
                    let len = packet.data.len();
                    let corrupt_idx = self.rng.lock().unwrap().gen_range_u32(0, len as u32) as usize;
                    packet.data[corrupt_idx] = (self.rng.lock().unwrap().gen_u32() % 256) as u8;
                }

                // Duplication
                if self.rng.lock().unwrap().gen_f32() < *duplication_chance {
                ...
                // Latency with range
                let latency = self.rng.lock().unwrap().gen_range_u32(latency_range_ms.0, latency_range_ms.1 + 1);
                packet.deliver_at = Instant::now() + Duration::from_millis(latency as u64);
